...
UserModels.edituser = async (data, id) => {
  try {
    ...
    for(var i=0;data.group.length>i;i++){
      ...
      if(nuevo){
        await modeluser_group.create({id_group_user_groups:data.group[i],id_user_user_groups:id})
...
UserModels.deleteuser= async (data) => {
  try{
    ...
    const insertar13 = await modeluser_group.deleteMany( { id_user_user_groups: data.id } )
    if (insertar === "error") {
      console.log("ERROR");
      return (insertar1 = false);
    } else {
...
myAggregate = await modeluser.aggregate([
  {
    $match: {
      ...
      first_name_user:data.firstname,
